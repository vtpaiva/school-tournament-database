...
class Estudante:
    ...
    def get_from_input(cls):
        ...
        if not valida_altura(altura=altura):
            raise ValueError("Valor de altura inválido. (O valor deve estar entre 1 e 3 metros)")

        peso = input("Digite o peso do estudante (em kg): ")
        if not valida_peso(peso=peso):
            raise ValueError("Valor de peso inválido. (O valor deve estar entre 20 e 200 kilos)")
        ...
        classificacao_altura = continuo_para_discreto(
            campo=0, valor=float(altura))
        classificacao_peso = continuo_para_discreto(campo=1, valor=float(peso))
