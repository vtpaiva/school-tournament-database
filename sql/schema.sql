CREATE TYPE ALTURA_MODALIDADE AS ENUM ('MUITO BAIXO', 'BAIXO', 'MEDIO-BAIXO', 'MEDIO', 'ALTO', 'MUITO ALTO', 'QUALQUER');
CREATE TYPE PESO_MODALIDADE AS ENUM ('MUITO LEVE', 'LEVE', 'MEDIO', 'PESADO', 'MUITO PESADO', 'EXTREMAMENTE PESADO', 'QUALQUER');
CREATE TYPE ALTURA_ESTUDANTE AS ENUM ('MUITO BAIXO', 'BAIXO', 'MEDIO-BAIXO', 'MEDIO', 'ALTO', 'MUITO ALTO');
...
CREATE TYPE SEXOS_MODALIDADE AS ENUM ('M', 'F');

CREATE TABLE Modalidade (
    ID SERIAL PRIMARY KEY,
    ESPORTE VARCHAR(16) NOT NULL,
    SEXO SEXOS_MODALIDADE NOT NULL,
...
CREATE TABLE Premio_individual (
    ID_COMPETICAO INT,
    CRITERIO VARCHAR(16),
    REGRA_COLOCACAO CHAR(1) NOT NULL,
    PREMIACAO SMALLINT DEFAULT 0,
    
    CONSTRAINT PK_PREMIO_INDIVIDUAL PRIMARY KEY(ID_COMPETICAO, CRITERIO),
    CONSTRAINT FK_COMPETICAO FOREIGN KEY(ID_COMPETICAO)
    REFERENCES Competicao(ID),
    CONSTRAINT CK_PREMIO_INDIVIDUAL_PREMIACAO CHECK (
        PREMIACAO >= 0
    ),
    CONSTRAINT CK_PREMIO_INDIVIDUAL_REGRA CHECK (
        REGRA_COLOCACAO IN ('+', '-')
    )
);
